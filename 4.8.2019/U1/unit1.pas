...
implementation
 ...
 procedure generate;
 var
   SJL,ANJ,NEJ,FRJ,SPJ,MAT,FYZ,CHE,INF,BIO,GEG,DEJ,OBN,UKL,TSV :textFile;
 begin
   with Form1 do begin
     Button1.Visible:=False;
     Edit1.Visible:=False;
     ...
     Memo1.Lines.Add('Ale nefunguj :)');
   end;
   assignFile(SJL,'SJL.txt');
   assignFile(ANJ,'ANJ.txt');
   assignFile(NEJ,'NEJ.txt');
   assignFile(FRJ,'FRJ.txt');
   assignFile(SPJ,'SPJ.txt');
   assignFile(MAT,'MAT.txt');
   assignFile(FYZ,'FYZ.txt');
   assignFile(CHE,'CHE.txt');
   assignFile(INF,'INF.txt');
   assignFile(BIO,'BIO.txt');
   assignFile(GEG,'GEG.txt');
   assignFile(DEJ,'DEJ.txt');
   assignFile(OBN,'OBN.txt');
   assignFile(UKL,'UKL.txt');
   assignFile(TSV,'TSV.txt');
   rewrite(SJL);
   rewrite(ANJ);
   rewrite(NEJ);
   rewrite(FRJ);
